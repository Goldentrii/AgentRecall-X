...
if (APPLY) {
  if (diff.changed) {
    ...
    // fix6-locks integration (2026-09-12): withLock is async on main — await
    // it (top-level await, ESM) so the locked write completes before promotion.
    await withLock("insights-index", () => {
  ...
  if (diff.changed) {
    diff.promotion = await promoteConfirmedInsights(PROMOTION_THRESHOLD);
  }
}
