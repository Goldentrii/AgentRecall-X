...
- **(2026-07-27) `auto-name.ts` TYPE_SIGNALS has the same vocabulary-vs-condition bug just fixed in `journal-sig-theme.ts`.** `TYPE_SIGNALS["tool-config"]` includes a bare `/\bmcp\b/i` signal; `detectContentType()` needs only 2 of 7 signals in a category to fire, so any novada-mcp summary that also says "server" (near-universal — "MCP server", "mcp.novada.com") gets content-type `tool-config` regardless of whether the session was actually about tool configuration (real slugs observed: `tool-config-090-nov-close`, `tool-config-tow2-owner-key`, `tool-config-batch-fix-handbook` — none are about config/setup/install). Deferred rather than fixed alongside the theme/sig fix because the blast radius is materially larger: `generateSlug`/`detectContentType` is shared infra called from `session.ts` (journal), `knowledge-write.ts`, `palace-write.ts`, `smart-remember.ts`, `check.ts`, `journal-capture.ts`, and `skill-propose.ts` — not journal-only — and a correct fix needs the same real-corpus-grounded audit across all 8 `TYPE_SIGNALS` categories (and their downstream keyword-extraction interaction), which is its own scoped change, not a small patch.
