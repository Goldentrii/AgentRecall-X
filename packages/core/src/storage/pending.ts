 ...
 */
export function listPendingCorrections(project: string): PendingRecord[] {
  try {
    return withLock(`pending-${project}`, () => {
 ...
 */
export function stagePendingCorrection(project: string, input: StagePendingInput): StagePendingResult {
  ...
  try {
    return withLock(`pending-${project}`, (): StagePendingResult => {
      const dir = pendingDir(project);
      ensureDir(dir);
...
export function resolvePendingCorrection(
  ...
  try {
    return withLock(`pending-${project}`, (): ResolvePendingResult => {
      const dir = pendingDir(project);
