...
export async function check(input: CheckInput): Promise<CheckResult> {
  ...
  if (typeof hc === "string") {
    if (!hc.trim()) {
      ...
      const stageRes = stagePendingCorrection(slug, {
        kind: "correction",
    ...
    if (hc.resolution === "reject") {
      if (!hc.pending_id) {
        ...
        const res = resolvePendingCorrection(slug, hc.pending_id, "reject", { reason: hc.why });
      ...
      if (!validation.ok) {
        ...
        try {
          ...
          if (!writeResult.written) {
            ...
            activatedRule = rule;
            if (hc.pending_id) {
              const res = resolvePendingCorrection(slug, hc.pending_id, "promote", {
