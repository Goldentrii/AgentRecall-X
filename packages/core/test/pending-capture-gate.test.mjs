...
describe("L1: string human_correction is STAGED, never active (R1 pin)", () => {
  it("string form lands in corrections/_pending/ and the ACTIVE ledger stays empty", async () => {
    ...
    const pending = await listPendingCorrections(project);
  ...
  it("whitespace-only string human_correction reports rejected_junk (never a silent no-op)", async () => {
    ...
    assert.equal((await listPendingCorrections(project)).length, 0);
...
describe("L1: structured human_correction {rule, why, applies_when}", () => {
  ...
  it("fragment applies_when ([\"good,\",\"then\",\"don't\"]) fails completeness → staged pending", async () => {
    ...
    assert.equal((await listPendingCorrections(project)).length, 1);
  });
...
describe("pending review loop: promote by id → active; reject → _pending/_rejected", () => {
  it("check() structured with pending_id promotes the staged item to the active ledger", async () => {
    ...
    assert.equal((await listPendingCorrections(project)).length, 0, "the staged item must leave the pending store on promote");
  });
...
describe("pending lifecycle: cap + TTL with provenance", () => {
  it("stage beyond PENDING_CAP evicts the oldest into _pending/_rejected.jsonl", async () => {
    ...
    const res = await stagePendingCorrection(project, {
      kind: "correction",
  ...
  it("TTL-expired items are pruned on read into _pending/_rejected.jsonl", async () => {
  ...
  it("resolvePendingCorrection: unknown id → success:false, never throws", async () => {
    const r = await resolvePendingCorrection("resolve-missing", "nope", "reject", { reason: "n/a" });
...
describe("distilled rule identity merge", () => {
  ...
  it("HIGH-2 pin: an empty identity never merges — distinct symbol-only captures stay distinct pending rows", async () => {
    const project = "empty-identity-no-merge";
    const a = await stagePendingCorrection(project, {
      kind: "correction", channel: "check_string",
      text: "!!!!!!!!!!!! ????", reason: "test",
    });
    const b = await stagePendingCorrection(project, {
      kind: "correction", channel: "check_string",
      text: "@@@@@@@@@@@@ ****", reason: "test",
    });
    // Whatever the hard-noise gate decides for these, they must NEVER fold
    // into one row via the empty identity.
    const rows = await listPendingCorrections(project);
  ...
  it("merge absorbs the incoming structured form's applies_when (union, like tags)", async () => {
    ...
    const b = await writeCorrection(project, {
  ...
  it("a reworded restatement of the SAME rule merges (proof_count++), via writeCorrection", async () => {
    const project = "merge-reworded";
    const a = await writeCorrection(project, {
  ...
  it("CJK: full-width punctuation / internal-whitespace variants merge", async () => {
  ...
  it("DISTINCT rules still never merge (proxy.ts vs middleware.ts)", async () => {
