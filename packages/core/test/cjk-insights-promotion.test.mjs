...
describe("fix3: CJK confirm-first chain (addIndexedInsight)", () => {
  ...
  it("two same-meaning zh titles merge into one entry at confirmed_count 2", () => {
    const first = addIndexedInsight({
    ...
    const second = addIndexedInsight({
  ...
  it("a third confirmation reaches the promotion threshold (confirmed_count 3)", () => {
    const third = addIndexedInsight({
  ...
  it("a DISTINCT zh title is admitted as a separate entry", () => {
    const b = addIndexedInsight({
  ...
  it("mixed zh/en titles merge on the combined Han+ASCII identity", () => {
    const m1 = addIndexedInsight({
      title: MIXED_1, source: "test", applies_when: [], severity: "minor",
    });
    assert.ok(m1);
    const m2 = addIndexedInsight({
  ...
  it("punctuation-only titles never merge with anything (no match-anything key)", () => {
    const lenBefore = readInsightsIndex().insights.length;
    const p1 = addIndexedInsight({ title: PUNCT, source: "test", applies_when: [], severity: "minor" });
    assert.ok(p1);
    assert.equal(p1.confirmed_count, 1);
    const p2 = addIndexedInsight({ title: "!!!???", source: "test", applies_when: [], severity: "minor" });
    assert.ok(p2);
  ...
  it("ASCII regression: near-identical English titles still merge; unrelated ones still do not", () => {
    const lenBefore = readInsightsIndex().insights.length;
    const e1 = addIndexedInsight({
...
describe("fix3: CJK promotion into awareness (promoteConfirmedInsights)", () => {
  ...
  it("promotes a zh insight at confirmed_count >= 3 into awareness end-to-end", () => {
  ...
  it("KNOWN LIMITATION: an unspaced pure-kana title is still rejected by the awareness gate", () => {
