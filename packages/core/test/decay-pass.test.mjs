...
describe("Wave 3 — in-repo decay pass", () => {
  ...
  it("a stale skill is flagged archived:true but NEVER deleted from disk", async () => {
    ...
    const report = await decay.runDecayPass(PROJECT, { dryRun: false });
  ...
  it("keystone and corrections/critical_path rooms are skipped (never archived)", async () => {
    // Make a keystone room very stale → would normally be a candidate.
    const old = new Date(Date.now() - 120 * 24 * 3600 * 1000).toISOString();
    rooms.createRoom(PROJECT, "key-room", "Key Room", "load-bearing");
    await rooms.updateRoomMeta(PROJECT, "key-room", { keystone: true, updated: old, last_accessed: old });
    rooms.createRoom(PROJECT, "corrections", "Corrections", "human corrections");
    await rooms.updateRoomMeta(PROJECT, "corrections", { updated: old, last_accessed: old });
  ...
  it("runDecayPass never throws on a project with no palace", async () => {
