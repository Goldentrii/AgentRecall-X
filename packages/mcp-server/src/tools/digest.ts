...
export function register(server: McpServer): void {
  server.registerTool("digest", {
    ...
    if (action === "invalidate") {
      ...
      // fix9: markStale is async-only (the Atomics.wait sync twin that review
      // MEDIUM-2 warned about — it would have frozen every in-flight tool
      // call on this server under digest-lock contention — is retired).
      const found = await markStale(resolvedProject, params.digest_id, params.reason ?? "manually invalidated", params.global);
      if (!found) {
